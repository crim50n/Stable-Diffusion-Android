            ...
            .subscribeBy(
                ...
                onSuccess = { samplers ->
                    updateGenerationState { state ->
                        ...
                        val defaultSampler = state.modelType.defaultSampler()
                        val selectedSampler = if (allSamplers.contains(defaultSampler)) {
                            defaultSampler
                        } else {
                            allSamplers.firstOrNull() ?: ""
                        }
                        state.copyState(
                            availableSamplers = allSamplers,
                            selectedSampler = selectedSampler,
                            selectedScheduler = state.modelType.defaultScheduler(),
                            width = state.modelType.defaultWidth().toString(),
                            height = state.modelType.defaultHeight().toString(),
                        )
                    }
                }
            )
